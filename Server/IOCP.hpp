...
public:
	...
	bool SendMsg(PacketData* pPacket_)
	{
		...
		Connection* pConnection = GetConnection(pPacket_->m_SessionIndex);
	...
	bool StoreMsg(const int connectionIndex_, char* msg_, unsigned int size_)
	{
		Connection* conn = GetConnection(connectionIndex_);
		return conn->StorePartialMessage(msg_, size_);
	}

	unsigned int GetMsg(const int connectionIndex_, char* out_)
	{
		Connection* conn = GetConnection(connectionIndex_);
		return conn->GetReqMessage(out_);
	}
