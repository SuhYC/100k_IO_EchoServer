...
public:
	...
	void SendMsg(PacketData* pData_)
	{
		...
		m_SendBuffer.enqueue(pData_->pData, pData_->dataSize);
	...
	unsigned int GetReqMessage(char* out_)
	{
		...
		unsigned int len = m_RecvBuffer.dequeue(cstr, MAX_SOCKBUF);
		...
		unsigned int remainlen = CheckStringSize(cstr, len, out_);
