...
# 의문점
C++ to C++에서 궁금한 것이 생겨 다수의 더미클라이언트를 작성해보고자 했다. <br/>
멀티스레드로 기존의 클라이언트 코드를 실행하여 다수의 더미클라이언트를 다음과 같이 실행하였다. <br/>
![DummyCode](https://github.com/SuhYC/100k_IO_EchoServer/blob/main/DummyTest.png) <br/>
그런데 그러던 중 클라이언트의 갯수가 3일때, 기존 클라이언트 수가 1일 때보다 매우 빠른 속도로 IO가 처리되는 것을 확인하게 되었다. <br/>
기존 결과에서 100만개의 패킷을 처리하는데 평균 10초 이상이 소요되었던 것에 비해 클라이언트 3개가 연결된 상태에서는 오히려 100만개의 패킷을 각각 클라이언트에서 처리하는데 2.5초 내외만 소요되는 것. <br/>
이것저것 테스트하다보니 원인을 알 수 없는 매우 빠른 처리가 가능한 지점들이 발견되었고 이는 IOCP 서버에서 워커스레드 수 제한과 관계가 있었다. <br/>
워커스레드 수에 근접할 수록 이상하리만치 빠른 속도로 처리가 가능했던것. <br/>
다음은 워커스레드 수를 1로 제한하고 100만패킷을 처리하는 단일 클라이언트에서 나온 결과이다. <br/>
![DummyResult](https://github.com/SuhYC/100k_IO_EchoServer/blob/main/DummyTestResult.png) <br/>
초당 82만개의 패킷을 처리한 것.. <br/>
심지어는 1000만개의 패킷을 처리하는데 10초가 걸리지 않은 회차도 있었다.. <br/>
